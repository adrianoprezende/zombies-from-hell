...
public class MainThread extends Thread {
	...
	public MainThread(MainGamePanel panel) {
		mainGamePanel = panel;
		holder = mainGamePanel.getHolder();
	}
		...
		while(running) {
			try {
				...
				synchronized(holder) {
					...
					mainGamePanel.updateGame();
					mainGamePanel.onDraw(canvas, elapsedTime);
					mainGamePanel.gameOver();
					...
					while (sleepTime < 0 && framesSkipped < MAX_FRAME_SKIPS) {
						// we need to catch up
						mainGamePanel.updateGame(); // update without rendering
...
//          Log.d(TAG, "Average FPS:" + df.format(averageFps));
            mainGamePanel.setAvgFps("FPS: " + df.format(averageFps));
        }
    }
